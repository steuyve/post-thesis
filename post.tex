...
Let $d = 2m+1$ be odd and $B_2^d$ be the closed $d$-dimensional Euclidean ball. Then
\begin{equation}
\frac{d^2}{dt^2}\text{Mag}\left(tB_2^d\right)\big\vert_{t=0} \geq \frac{1}{2}V_1^2+V_1\left(\frac{3(m-1)}{2m+1}\right)-\frac{2(12m^3+4m^2-6m-1)}{(2m-1)(2m+1)}.
\end{equation}
Moreover, equality is achieved when $d=3,5,7$.
\end{prop}
